...
def build_lj_plotly_zscore(points: List[Dict[str, Any]], title: str) -> go.Figure:
    """
    Standardized Levey-Jennings: plots Z-SCORE instead of raw result, so
    the ±1/2/3 SD bands sit at fixed y=±1/±2/±3 no matter what. This is
    the right way to show a range that spans more than one control lot —
    raw-unit charts silently mix two different reference scales (e.g.
    Lot A mean 100/SD 3, Lot B mean 110/SD 4) on one axis, which is
    misleading. Each point's z was computed against ITS OWN lot's mean/SD
    at entry time, so this view is always lot-correct.
    """
    x = _x_labels(points)
    y = [p["z"] for p in points]
    colors = [STATUS_COLORS.get(p["status"], "#1976d2") for p in points]
    hover = [
        f"{p['date']} run {p['run_number']}<br>Result: {p['result']}<br>"
        f"Z: {p['z']:.2f}<br>Status: {p['status'].upper()}"
    ...
    band_specs = [(3, "rgba(198,40,40,0.06)"), (2, "rgba(249,168,37,0.08)"), (1, "rgba(46,125,50,0.06)")]
    for n, color in band_specs:
        fig.add_hrect(y0=-n, y1=n, fillcolor=color, line_width=0)
    for n in (1, 2, 3):
        fig.add_hline(y=n, line_dash="dot", line_color="gray", opacity=0.5,
                       annotation_text=f"+{n}SD", annotation_position="right")
        fig.add_hline(y=-n, line_dash="dot", line_color="gray", opacity=0.5,
                       annotation_text=f"-{n}SD", annotation_position="right")
    ...
    fig.update_layout(
        title=title + " (standardized Z-score)",
        xaxis_title="Run",
        yaxis_title="Z-score (SD units)",
...
def build_lj_matplotlib_zscore(points: List[Dict[str, Any]], title: str) -> bytes:
    """Static PNG version of the standardized Z-score chart, for embedding
    in the PDF report — see build_lj_plotly_zscore for why this exists."""
    x = list(range(len(points)))
    y = [p["z"] for p in points]
    colors = [STATUS_COLORS.get(p["status"], "#1976d2") for p in points]
    labels = _x_labels(points)
    ...
    for n, alpha in ((3, 0.05), (2, 0.08), (1, 0.05)):
        ax.axhspan(-n, n, color="gray", alpha=alpha)
    for n in (1, 2, 3):
        ax.axhline(n, color="gray", linestyle=":", linewidth=0.8)
        ax.axhline(-n, color="gray", linestyle=":", linewidth=0.8)
        ax.text(len(x) - 0.5, n, f"+{n}SD", fontsize=7, va="bottom", color="gray")
        ax.text(len(x) - 0.5, -n, f"-{n}SD", fontsize=7, va="top", color="gray")
    ax.axhline(0, color="black", linewidth=1.2)
    ax.text(len(x) - 0.5, 0, "Mean", fontsize=7, va="bottom")
    ...
    for xi, p in zip(x, points):
        if p["status"] in ("reject", "warning") and p.get("rule_names"):
            ax.annotate("/".join(p["rule_names"]), (xi, p["z"]),
                        textcoords="offset points", xytext=(0, 8),
                        fontsize=6.5, color=STATUS_COLORS[p["status"]], ha="center")

    ax.set_ylim(-4, 4)
    ax.set_title(title + " (standardized Z-score)", fontsize=11)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=45, ha="right", fontsize=6.5)
    ax.set_ylabel("Z-score (SD units)")
    fig.tight_layout()
